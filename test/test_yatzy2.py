...
from yatzy.yatzy2 import Yatzy
...
def test_test_1s(calculator):
    assert (1 == calculator.score([1, 2, 3, 4, 5], "ONE"))
    assert (2 == calculator.score([1, 2, 1, 4, 5], "ONE"))
    assert (0 == calculator.score([6, 2, 2, 4, 5], "ONE"))
    assert (4 == calculator.score([1, 2, 1, 1, 1], "ONE"))


def test_twos(calculator):
    assert (4 == calculator.score([1, 2, 3, 2, 6], "TWO"))
    assert (10 == calculator.score([2, 2, 2, 2, 2], "TWO"))
...
def test_three_of_a_kind(calculator):
    assert (9 == calculator.score([3, 3, 3, 4, 5], "THREE_OF_KIND"))
    assert (15 == calculator.score([5, 3, 5, 4, 5], "THREE_OF_KIND"))
    assert (9 == calculator.score([3, 3, 3, 3, 5], "THREE_OF_KIND"))
